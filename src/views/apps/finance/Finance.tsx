...
const Finance = () => {
  ...
  const handleNewPocketClose = () => {
    setOpenNewPocket(false);
    setNewPocketForm({
      name: '',
      type: '',
      amount: 0,
  ...
  const pendingPayments = mockProjects
    .filter(p => p.payment_status !== 'Lunas')
  ...
  return (
    <PageContainer title="Dashboard Keuangan" description="Dashboard keuangan komprehensif">
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid size={12}>
              <TextField 
                ...
                onChange={(e) => setNewCardForm({ ...newCardForm, balance: Number(e.target.value) })}
              />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleNewCardClose}>Batal</Button>
          <Button variant="contained" onClick={handleNewCardClose}>
            Simpan
          </Button>
        </DialogActions>
      </Dialog>
      ...
      {/* Dialog Kantong Baru */}
      <Dialog open={openNewPocket} onClose={handleNewPocketClose} maxWidth="sm" fullWidth>
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField 
                ...
                label="Saldo Awal" 
                type="number" 
                value={newPocketForm.amount}
                onChange={(e) => setNewPocketForm({ ...newPocketForm, amount: Number(e.target.value) })}
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField 
                ...
                type="number" 
                value={newPocketForm.goalAmount}
                onChange={(e) => setNewPocketForm({ ...newPocketForm, goalAmount: Number(e.target.value) })}
              />
            </Grid>
            <Grid size={12}>
              <TextField 
        ...
        <DialogActions>
          <Button onClick={handleNewPocketClose}>Batal</Button>
          <Button variant="contained" onClick={handleNewPocketClose}>
            Buat Kantong
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
            ...
            <Grid size={12}>
              <FormControl fullWidth>
                ...
                <Select 
                  ...
                  onChange={(e) => setNewTransactionForm({ ...newTransactionForm, type: e.target.value })}
                >
            ...
            <Grid size={12}>
              <FormControl fullWidth>
                ...
                <Select 
                  ...
                  value={newTransactionForm.method}
                  onChange={(e) => setNewTransactionForm({ ...newTransactionForm, method: e.target.value })}
                >
            ...
            <Grid size={12}>
              <FormControl fullWidth>
                ...
                <Select 
                  ...
                  value={newTransactionForm.cardId}
                  onChange={(e) => setNewTransactionForm({ ...newTransactionForm, cardId: e.target.value })}
                >
                  {cards.map((card) => (
                    <MenuItem key={card.id} value={card.id}>
                      {card.bank_name || card.bankName} - **** {card.last_four_digits || card.lastFourDigits}
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
        ...
        <DialogActions>
          <Button onClick={handleNewTransactionClose}>Batal</Button>
          <Button variant="contained" onClick={handleNewTransactionClose}>
            Simpan
          </Button>
        </DialogActions>
      </Dialog>
