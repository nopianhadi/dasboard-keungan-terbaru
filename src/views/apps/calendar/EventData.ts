...
const generateEvents = (): EventType[] => {
  ...
  mockLeads.forEach((lead: Lead) => {
    if (lead.lastContact && lead.status !== 'Ditolak' && lead.status !== 'Dikonversi') {
      ...
      events.push({
        ...
        allDay: true,
        color: lead.priority === 'Tinggi' ? 'red' : 'warning',
        type: 'lead',
        description: `Follow up lead - ${lead.projectType || 'General'} | Priority: ${lead.priority}`,
      });
    }
  });
