...
const PocketCard: React.FC<PocketCardProps> = ({ pocket, onMenuClick }) => {
  ...
  const progress = (pocket.goal_amount || pocket.goalAmount) ? (pocket.amount / (pocket.goal_amount || pocket.goalAmount)) * 100 : 0;
  ...
  return (
    ...
    >
      <CardContent sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        {(pocket.goal_amount || pocket.goalAmount) && (
          <Box mt="auto">
            <Stack direction="row" justifyContent="space-between" mb={1}>
              <Typography variant="caption" color="textSecondary" fontSize="0.75rem">
                Target: {formatCurrency(pocket.goal_amount || pocket.goalAmount)}
              </Typography>
        ...
        {pocket.description && !(pocket.goal_amount || pocket.goalAmount) && (
